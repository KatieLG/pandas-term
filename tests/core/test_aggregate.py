...
from pandas_cli.core import aggregate


def test_value_counts(sample_df: pd.DataFrame) -> None:
    """Test value counts for a column."""
    result = aggregate.value_counts(sample_df, "city")
    assert len(result) == 3
...
def test_value_counts_normalize(sample_df: pd.DataFrame) -> None:
    ...
    result = aggregate.value_counts(sample_df, "department", normalize=True)
    assert len(result) == 2
...
def test_group_by_sum(sample_df: pd.DataFrame) -> None:
    ...
    result = aggregate.group_by(sample_df, ["department"], "salary", "sum")
    assert len(result) == 2
...
def test_group_by_mean(sample_df: pd.DataFrame) -> None:
    ...
    result = aggregate.group_by(sample_df, ["city"], "age", "mean")
    assert len(result) == 3
...
def test_group_by_count(sample_df: pd.DataFrame) -> None:
    ...
    result = aggregate.group_by(sample_df, ["city"], "name", "count")
    assert len(result) == 3
